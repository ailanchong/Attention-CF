...
def main():
    np.random.seed(2018)
    dater = Data_Factory()
    R, D_all = dater.preprocess("./ml-1m_ratings.dat", "./ml_plot.dat", 3, 300, 0.5, 8000)
    train, valid, test = dater.split_data(0.2, R)
    R_lil = R.tolil()
    file_trian = open("data/train", 'w')
    file_val = open("data/val", 'w')
    file_test = open("data/test", 'w')
    for line in train:
        data = {}
        data['usridx'] = str(line[0])
        data['proidx'] = str(line[1])
        #print(D_all['X_sequence'][line[1]])
        data['sentence'] = D_all['X_sequence'][line[1]]
        data['sentenlen'] = len(D_all['X_sequence'][line[1]])
        data['rate'] = R_lil[line[0], line[1]]
        data = json.dumps(data)
        file_trian.write(data + '\n')
    file_trian.close()
    for line in valid:
        data = {}
        data['usridx'] = str(line[0])
